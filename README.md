<!--
1) Layout ideas and section inspiration:
   https://github.com/abhisheknaiidu/awesome-github-profile-readme?tab=readme-ov-file#descriptive-

2) Skill icons (SVG badges):
   https://github.com/tandpfun/skill-icons?tab=readme-ov-file#icons-list

3) GitHub stats card:
   https://github.com/anuraghazra/github-readme-stats
-->

## 👨🏽‍💻 Sobre mim

Sou Thalisson Souza, estudante de Ciência da Computação na UFSM, com foco em desenvolvimento backend. Possuo experiência em projetos reais desenvolvidos ao longo da graduação
e em ambientes profissionais colaborativos, atuando principalmente com APIs, automações e aplicações web.

- 🎓 Graduando em Ciência da Computação na Universidade Federal de Santa Maria (UFSM)
- 💻 Desenvolvedor Backend e gestor Administrativo-Financeiro na **CompAct**, Empresa Júnior de Tecnologia da UFSM
- 🚀 Experiência em projetos que automatizaram processos reais e reduziram trabalho manual.
- 🔗 Explore meus projetos no <a href="https://github.com/Thalisson-Souza?tab=repositories" target="_blank">GitHub</a>

---

## ⚙️ Tecnologias

<div align="center">
	<code><img width="40" src="https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/java.png" alt="Java" title="Java"/></code>
	<code><img width="40" src="https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/spring_boot.png" alt="Spring Boot" title="Spring Boot"/></code>
	<code><img width="40" src="https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/node_js.png" alt="Node.js" title="Node.js"/></code>
	<code><img width="40" src="https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/c.png" alt="C" title="C"/></code>
	<code><img width="40" src="https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/react.png" alt="React" title="React"/></code>
	<code><img width="40" src="https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/javascript.png" alt="JavaScript" title="JavaScript"/></code>
	<code><img width="40" src="https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/html.png" alt="HTML" title="HTML"/></code>
	<code><img width="40" src="https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/css.png" alt="CSS" title="CSS"/></code>
	<code><img width="40" src="https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/git.png" alt="Git" title="Git"/></code>
</div>

---

<!-- ## 🚀 Projetos em destaque

### 🌿 Automação de Agendamento – Jardim Botânico UFSM

Automação web para agendamento de visitas guiadas e gestão de monitores, eliminando o trabalho manual da equipe.
🔗 [Repositório](https://github.com/Thalisson-Souza)


### 🌱 BSE Platform

Plataforma web educacional para estudantes com Benefício Socioeconômico (BSE) da UFSM.
🔗 [Repositório](https://github.com/Thalisson-Souza)


### 🚗 Parking Control API

API REST para controle de vagas de estacionamento.
🔗 [Repositório](https://github.com/Thalisson-Souza)
 -->
